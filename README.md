...
🔐 **Современная криптографическая система**

Данный проект демонстрирует защищённую клиент-серверную систему на языке Go с использованием:
- **AES-256-CBC + HMAC-SHA256** для шифрования и проверки целостности
- **ECDSA** для цифровых подписей и проверки подлинности
- **ECDH + RSA** для безопасного обмена ключами
- **Nonce и timestamp** для защиты от replay attacks
- **Постоянный мониторинг целостности** данных
- **Адаптивные профили безопасности** для разных типов клиентов
- **Измерение производительности** всех операций
- **Тесты устойчивости к атакам** (brute-force, timing, replay)
- **Нагрузочные тесты** для Mobile/Web/Desktop/Server клиентов 
- **Фиксация производительности** при различных условиях 
- **Полная документация** алгоритма и использования 
...
6. **Интегральный показатель эффективности** - вычислялись по следующим фромулам: Нормализация параметров и Веса (смотри далее)
...
**Все тесты атак отражены** (0% успешных атак из  10+ млн попыток)
...
## Защита от атак (полная реализация с улучшениями)
...
```
Тип атаки                      Попыток        Успешных      Статус        
-----------------------------------------------------------------------
Brute Force AES               14,978,535         0         ОТРАЖЕНО     
Enhanced Timing Attack HMAC       20,000         0         ОТРАЖЕНО     
Enhanced Replay Attack                 3         0         ОТРАЖЕНО      
Enhanced Concurrent Brute       Variable         0         ЗАЩИЩЕНО       
Signature Forgery                  1,000         0         ОТРАЖЕНО      
-----------------------------------------------------------------------
ИТОГО:                       10,000,000+         0       100% ЗАЩИТА  
```
...
2. **TimingStats** - анализ времени HMAC операций:
...
- 10+ млн попыток атак отражено (0% успешных) ✓
